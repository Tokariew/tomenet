...
Server is build on fedora, it will create archive with all files which are
...
- make-tomenet-server - script which is runned by systemd on my nas to compile
tomenet, it move archived build to my nextcloud "webdav" using rclone, and then
sync tomenet folder to mega and dropbox.

- tomenet-compiler.service this is prehistoric file generated by podman which
run above script

- tomenet-compiler.timer - systemd timer to run above unit file weekly on
mondays at 12:15. If for some reason my nas was offline during that time, it
should run after going online
